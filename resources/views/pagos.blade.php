...
<body>
    <div class="dashboard-wrapper">
        ...
        <main class="main-content" id="mainContent">
            ...
            {{-- Header del layout (hamburguesa + notificaciones + logo) --}}
            <header class="header">
            ...
            <div class="container">
                ...
                {{-- ✅ Segundo header renombrado a .page-header (ya no choca con .header) --}}
                <div class="page-header">
                    <div class="page-header-content">
                        <div class="page-header-title">
                            <i class="fas fa-dollar-sign"></i>
                            <div>
                                <h2>Control y registro de pagos judiciales</h2>
                            </div>
                        </div>
                        <div class="page-header-actions">
                            ...
                            <div class="total-badge">
                                ...
                                <div class="value">
                                    <span id="totalProcesos">{{ count($procesosData) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="pagination-desktop"
                    style="display:flex; justify-content:center; gap:0.3rem; margin-top:1rem;">
                </div>
    ...
    <script>
        ...
        let terminoBusqueda = '';
        const esAbogado = @json($esAbogado);

        const urlParams = new URLSearchParams(window.location.search);
        const procesoIdNotificacion = urlParams.get('proceso');

        if (procesoIdNotificacion) {
            const procesoEncontrado = procesos.find(p => p.id == procesoIdNotificacion);
            if (procesoEncontrado) {
                ...
                setTimeout(() => {
                    ...
                    if (card) {
                        card.style.border = '3px solid #facc15';
        ...
        function renderizarProcesos() {
            ...
            let procesosFiltrados = procesos.filter(p => {
                if (filtroActual === 'pendientes' && (!p.requiere_pago || p.porcentaje >= 100)) return false;
                if (filtroActual === 'pagados' && p.porcentaje < 100) return false;
                if (terminoBusqueda) {
                ...
                ${proceso.requiere_pago ? `
                            <div class="pago-section">
                                <div class="pago-realizado">
                                    ...
                                    ${proceso.total_pendiente > 0 ? `
                        <div class="pago-item">
                            <div class="label">En Revisión</div>
                            <div class="value" style="color: #f59e0b;">${formatearPeso(proceso.total_pendiente)}</div>
                        </div>` : ''}
                                    <div class="pago-item">
                        ...
                        <div class="historial-contenido" id="historial-${proceso.id}" style="display:none;">
                            ${proceso.cuotas
                                ...
                                .map((pago, index) => `
                                            <div style="margin-top: 0.5rem; padding: 0.75rem; background: #f8fafc; border-radius: 8px; border-left: 3px solid #16a34a;">
                                                <div style="display: flex; justify-content: space-between; align-items: center; gap: 1rem;">
                                                    <div>
                                                        ...
                                                        <div style="font-size:0.85rem; color:#64748b; display:flex; gap:1rem; flex-wrap:wrap;">
                                                            <span><i class="fas fa-calendar"></i> ${formatearFecha(pago.fecha_pago)}</span>
                                                            <span><i class="fas fa-credit-card"></i> ${pago.forma_pago}</span>
                                                        </div>
                                                    </div>
                                                    <div style="display:flex; align-items:center; gap:0.5rem;">
                                                        ${pago.pago_estado === 'Pendiente'
                                                            ? `<span style="background:#facc15; color:#000; padding:4px 8px; border-radius:6px; font-size:0.8rem;"><i class="fas fa-clock"></i> Pendiente</span>`
                                                            : pago.pago_estado === 'Aprobado'
                                                                ? `<span style="background:#16a34a; color:white; padding:4px 8px; border-radius:6px; font-size:0.8rem;"><i class="fas fa-check-circle"></i> Aprobado</span>`
                                                                : pago.pago_estado === 'Rechazado'
                                                                    ? `<span style="background:#dc2626; color:white; padding:4px 8px; border-radius:6px; font-size:0.8rem;"><i class="fas fa-times-circle"></i> Rechazado</span>`
                                                                    : ''
                                                        }
                                                        ${pago.comprobante ? `
                                    ...
                                    <div style="margin-top:0.5rem; padding:0.5rem; background:#fee2e2; border-radius:6px; border-left:2px solid #dc2626; font-size:0.85rem;">
                                ...
                                ${proceso.falta_pagar > 0 || proceso.hay_pagos_rechazados ? `
                    <button class="btn-registrar" onclick="abrirModal(${proceso.id})">
                        <i class="fas fa-dollar-sign"></i>
                        Registrar nuevo pago
                    </button>` : `
                    <div style="margin-top: 1rem; padding: 1rem; background: #f0fdf4; border-left: 3px solid #16a34a; border-radius: 8px;">
                        <div style="color: #16a34a; font-weight: 600; display: flex; align-items: center; gap: 0.5rem;">
                            <i class="fas fa-check-circle"></i>
                            Todos los pagos están completos o en revisión
                        </div>
                    </div>`}
                            </div>` : ''}
            </div>
        ...
        async function registrarPago() {
            ...
            const totalPendiente = procesoSeleccionado.cuotas
                .filter(c => c.pago_estado === 'Pendiente')
                .reduce((sum, c) => sum + parseFloat(c.valor_pagado || 0), 0);

            const totalPagadoNum = parseFloat(procesoSeleccionado.total_pagado || 0);
            const valorEstimadoNum = parseFloat(procesoSeleccionado.valor_estimado || 0);
            const totalSiSeAprueban = totalPagadoNum + totalPendiente + valor;
            const esUltimoPago = totalSiSeAprueban >= valorEstimadoNum;
            ...
            const confirmacion = await Swal.fire({
                ...
                html: `
                    ...
                    ${esAbogado
                        ...
                        : (esUltimoPago
                            ? '<p style="color:#dc2626;font-weight:600;">⚠️ <strong>IMPORTANTE:</strong> El pago quedará en revisión. Una vez aprobado por el abogado, <strong>el proceso será ARCHIVADO automáticamente y no podrá ser modificado</strong>.</p>'
        ...
        function renderizarPaginacion(totalItems) {
            ...
            if (paginaActual === 1) {
                ...
                s.className = 'pagination-btn disabled';
                s.textContent = 'Anterior';
                pagContainer.appendChild(s);
            } else {
                const a = document.createElement('a');
                a.className = 'pagination-btn ajax-page';
                a.href = '#';
            ...
            if (start > 1) {
                ...
                f.className = 'pagination-btn ajax-page';
                f.href = '#';
                f.textContent = '1';
                ...
                pagContainer.appendChild(f);
                if (start > 2) {
                    const d = document.createElement('span');
                    d.className = 'pagination-btn disabled';
                    d.textContent = '...';
                    pagContainer.appendChild(d);
                }
            }
            ...
            for (let i = start; i <= end; i++) {
                if (i === paginaActual) {
                    const s = document.createElement('span');
                    s.className = 'pagination-btn active';
                    s.textContent = i;
                    pagContainer.appendChild(s);
                } else {
                    const a = document.createElement('a');
            ...
            if (end < totalPaginas) {
                ...
                l.className = 'pagination-btn ajax-page';
                l.href = '#';
                l.textContent = totalPaginas;
                l.addEventListener('click', e => {
                    e.preventDefault();
                    paginaActual = totalPaginas;
                    renderizarProcesos();
                });
                pagContainer.appendChild(l);
            }
        ...
        async function aprobarPago(id) {
            const proceso = procesos.find(p => p.cuotas.some(c => c.pago_id === id));
            if (proceso) {
                const pagoActual = proceso.cuotas.find(c => c.pago_id === id);
                const indexActual = proceso.cuotas.indexOf(pagoActual);
                const hayPendientesAnteriores = proceso.cuotas.slice(0, indexActual).some(c => c.pago_estado ===
                    'Pendiente');
                if (hayPendientesAnteriores) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'No se puede aprobar',
                        html: '<p>Debes aprobar los pagos anteriores primero.</p><p style="color:#dc2626;">Los pagos deben aprobarse en orden cronológico.</p>',
                        confirmButtonText: 'Entendido',
            ...
            try {
                ...
                Swal.fire('Error', 'Error de conexión al aprobar el pago', 'error');
            }
        }
        ...
        async function rechazarPago(id) {
            const {
                ...
                inputValidator: (value) => {
                    if (!value || value.length < 5) return 'El motivo es obligatorio (mínimo 5 caracteres)';
                }
            });
            if (!motivo) return;
            ...
            try {
                ...
                const data = await response.json();
                if (data.success) {
                    await Swal.fire('Rechazado', data.message, 'success');
                    location.reload();
                } else Swal.fire('Error', data.message, 'error');
            } catch (error) {
                Swal.fire('Error', 'Error de conexión al rechazar el pago', 'error');
            }
        }
